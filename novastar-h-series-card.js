                  ...
                  ${mt?v`
                        <div class="header-status">
                          ...
                          ${_?v`<span
                              ...
                              ></span>`:l}
                          ${O?this.renderHeaderBrightnessToggle(F,j,V,L,y,q):l}
                        </div>
                      `:l}
          ...
          ${x?v`
                ...
                ${m?v`
                      <div class="row input-row">
                        ...
                        ${ot?this.renderBrightnessControl(F,j,V,L,y,q):v`<span class="value">${m.state}${q}</span>`}
                      </div>
                    `:l}
              `:l}
        ...
        <div class="layer-source-modal-content" @click=${s=>s.stopPropagation()}>
          ...
          <div class="layer-source-modal-options">
            ${t.map(s=>v`
              <button
                type="button"
                class="layer-source-modal-option ${this.optionEquals(s,e)?"selected":""}"
                ?disabled=${r}
  ...
  `;renderLayoutPreview(t,e=!1){let r=t.screenWidth,s=t.screenHeight,i=this.fitLayersToViewport(t.layers,r,s).sort((g,m)=>g.z-m.z),n="#000000",o="#4a4a4a",c="#d9d9d9",d="#808080",p="#ffffff",h=this.getLayerSourceRows(),f=this.getLayerSourceLabelMap(),y=this.getEntityId("power_entity")??"switch.novastar_h2_power_screen_output",b=this.hass?.states[y],E=this.optimisticPowerState??b?.state,nt=!!b&&E!=="on";return v`
      <div class=${e?"layout-preview layout-preview--compact":"layout-preview"}>
        ...
        >
          ...
          ${i.map((g,m)=>{let $=this.resolveLayerSourceRow(h,g.id,m),L=this.resolveLayerSourceLabel(f,g.id,m)??g.source?.trim()??g.id,F=g.x+g.width/2,j=g.y+g.height/2,V=Math.min(g.width,g.height),ot=Math.max(54,r*.017),q=Math.max(V*.14,ot),H=Math.max(24,g.height*.38),C=Math.min(q,H),W=Math.max(18,C*.35),K=Math.max(32,g.width*.9),at=Math.max(1,Math.floor((K-W*2)/Math.max(1,C*.62))),M=L.length<=at?L:`${L.slice(0,Math.max(1,at-1))}\u2026`,ft=M.length*C*.62,P=Math.min(K,ft+W*2),x=Math.max(28,C*1.35),ct=F-P/2,yt=j-x/2,Y=Math.max(6,x*.25),u=Math.max(51,Math.min(138,V*.27)),dt=Math.max(10,u*.22),w=Math.max(g.x+4,g.x+g.width-u-dt),_=Math.min(g.y+dt,g.y+g.height-u-4),O=w+u*.22,mt=_+u*.34,X=u*.18,bt=u*.32,_t=[`${O+X},${_+u*.26}`,`${w+u*.68},${_+u*.16}`,`${w+u*.68},${_+u*.84}`,`${O+X},${_+u*.74}`].join(" "),B=w+u*.7,D=_+u*.5,Ht=g.audioOpen===!0,Ot=g.audioOpen===!1,G=Ht?"var(--success-color, #43a047)":Ot?"var(--secondary-text-color, #8a8a8a)":"color-mix(in srgb, var(--secondary-text-color, #8a8a8a) 55%, transparent)",lt=!!$&&!nt;return it`
              <g>
                <rect
                  class="layout-layer"
                  x=${g.x}
                ...
                <g>
                  ...
                  ${Ht?it`
                      <path
                        d=${`M ${B} ${D-u*.13} Q ${B+u*.12} ${D} ${B} ${D+u*.13}`}
                        fill="none"
                        stroke=${G}
                        stroke-width=${u*.06}
                      ...
                      ></path>
                      <path
                        d=${`M ${B+u*.1} ${D-u*.22} Q ${B+u*.28} ${D} ${B+u*.1} ${D+u*.22}`}
                        fill="none"
                        stroke=${G}
                        stroke-width=${u*.06}
                        stroke-linecap="round"
