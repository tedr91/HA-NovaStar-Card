                  ...
                  ${mt?v`
                        <div class="header-status">
                          ...
                          ${_?v`<span
                              ...
                              ></span>`:l}
                          ${D?this.renderHeaderBrightnessToggle(U,V,j,k,y,W):l}
                        </div>
                      `:l}
          ...
          ${P?l:w?v`
                  ...
                  ${m?v`
                        <div class="row input-row">
                          ...
                          ${ot?this.renderBrightnessControl(U,V,j,k,y,W):v`<span class="value">${m.state}${W}</span>`}
                        </div>
                      `:l}
                  ${E?v`
  ...
  `;renderLayoutPreview(t,e=!1){let r=t.screenWidth,s=t.screenHeight,i=this.fitLayersToViewport(t.layers,r,s).sort((g,m)=>g.z-m.z),n="#000000",a="#4a4a4a",c="#d9d9d9",d="#808080",p="#ffffff",h=this.getLayerSourceRows(),f=this.getLayerSourceLabelMap(),y=this.getEntityId("power_entity")??"switch.novastar_h2_power_screen_output",b=this.hass?.states[y],E=this.optimisticPowerState??b?.state,nt=!!b&&E!=="on";return v`
      <div class=${e?"layout-preview layout-preview--compact":"layout-preview"}>
        ...
        >
          ...
          ${i.map((g,m)=>{let $=this.resolveLayerSourceRow(h,g.id,m),k=this.resolveLayerSourceLabel(f,g.id,m)??g.source?.trim()??g.id,U=g.x+g.width/2,V=g.y+g.height/2,j=Math.min(g.width,g.height),ot=Math.max(54,r*.017),W=Math.max(j*.14,ot),M=Math.max(24,g.height*.38),L=Math.min(W,M),q=Math.max(18,L*.35),K=Math.max(32,g.width*.9),at=Math.max(1,Math.floor((K-q*2)/Math.max(1,L*.62))),N=k.length<=at?k:`${k.slice(0,Math.max(1,at-1))}\u2026`,ft=N.length*L*.62,P=Math.min(K,ft+q*2),w=Math.max(28,L*1.35),ct=U-P/2,yt=V-w/2,Y=Math.max(6,w*.25),u=Math.max(51,Math.min(138,j*.27)),dt=Math.max(10,u*.22),x=Math.max(g.x+4,g.x+g.width-u-dt),_=Math.min(g.y+dt,g.y+g.height-u-4),D=x+u*.22,mt=_+u*.34,X=u*.18,bt=u*.32,_t=[`${D+X},${_+u*.26}`,`${x+u*.68},${_+u*.16}`,`${x+u*.68},${_+u*.84}`,`${D+X},${_+u*.74}`].join(" "),B=x+u*.7,O=_+u*.5,Ht=g.audioOpen===!0,Dt=g.audioOpen===!1,G=Ht?"var(--success-color, #43a047)":Dt?"var(--secondary-text-color, #8a8a8a)":"color-mix(in srgb, var(--secondary-text-color, #8a8a8a) 55%, transparent)",lt=!!$&&!nt;return it`
              <g>
                <rect
                  class="layout-layer"
                  x=${g.x}
